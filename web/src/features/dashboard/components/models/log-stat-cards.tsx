    ...
    <div className='glass-card overflow-hidden rounded-2xl border border-border/80 shadow-xs backdrop-blur-xl'>
      <div className='divide-border/60 grid min-w-0 grid-cols-2 divide-x sm:grid-cols-3 lg:grid-cols-5'>
        {items.map((it, idx) => {
          ...
          if (loading) {
            valueContent = (
              <div className='mt-1.5 flex flex-col gap-1 sm:mt-2 sm:gap-1.5'>
            ...
            valueContent = (
              <>
                <div className='text-muted-foreground mt-1.5 font-mono text-base leading-tight font-bold tracking-tight tabular-nums sm:mt-2 sm:text-2xl sm:leading-normal'>
                  --
                </div>
                <div className='text-muted-foreground/40 mt-1 hidden font-mono text-xs md:block'>
                  {it.desc}
                </div>
              </>
            )
            ...
            valueContent = (
              <>
                ...
                <div className='text-muted-foreground/60 mt-1 hidden font-mono text-[11px] md:block'>
                  {it.desc}
                </div>
              </>
            )
          ...
          return (
            <div
              key={it.title}
              className={cn(
                'min-w-0 px-3 py-2.5 sm:px-5 sm:py-4 transition-colors hover:bg-muted/20',
                idx === items.length - 1 &&
                  items.length % 2 !== 0 &&
            ...
            >
              <div className='flex min-w-0 items-center gap-2'>
                ...
                <div className='text-muted-foreground truncate text-[11px] leading-4 font-mono font-medium tracking-wide uppercase sm:text-xs'>
                  {it.title}
                </div>
              </div>
