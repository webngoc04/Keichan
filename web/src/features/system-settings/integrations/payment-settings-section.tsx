...
const paymentSchema = z.object({
  ...
  PayosReturnURL: z.string(),
  PayosCancelURL: z.string(),
  PayosUnitPrice: z.coerce.number().min(0),
  PayosMinTopUp: z.coerce.number().min(0),
  NowpaymentsApiUrl: z.string(),
...
export function PaymentSettingsSection({
  ...
  const onSubmit = async (values: PaymentFormValues) => {
    const sanitized = {
      ...
      PayosClientId: values.PayosClientId.trim(),
      PayosApiKey: values.PayosApiKey.trim(),
      PayosChecksumKey: values.PayosChecksumKey.trim(),
      PayosReturnURL: removeTrailingSlash(values.PayosReturnURL.trim()),
      PayosCancelURL: removeTrailingSlash(values.PayosCancelURL.trim()),
      PayosUnitPrice: values.PayosUnitPrice,
      PayosMinTopUp: values.PayosMinTopUp,
      NowpaymentsApiUrl: removeTrailingSlash(
        values.NowpaymentsApiUrl.trim() ||
    ...
    const initial = {
      ...
      PayosClientId: initialRef.current.PayosClientId.trim(),
      PayosApiKey: initialRef.current.PayosApiKey.trim(),
      PayosChecksumKey: initialRef.current.PayosChecksumKey.trim(),
      PayosReturnURL: removeTrailingSlash(
        initialRef.current.PayosReturnURL.trim()
      ),
      PayosCancelURL: removeTrailingSlash(initialRef.current.PayosCancelURL.trim()),
      PayosUnitPrice: initialRef.current.PayosUnitPrice,
      PayosMinTopUp: initialRef.current.PayosMinTopUp,
      NowpaymentsApiUrl: removeTrailingSlash(
        initialRef.current.NowpaymentsApiUrl.trim() ||
          'https://api.nowpayments.io'
      ),
    ...
    if (sanitized.PayosClientId !== initial.PayosClientId) {
      updates.push({ key: 'PayosClientId', value: sanitized.PayosClientId })
    }

    if (sanitized.PayosApiKey && sanitized.PayosApiKey !== initial.PayosApiKey) {
      updates.push({ key: 'PayosApiKey', value: sanitized.PayosApiKey })
    }

    if (
      sanitized.PayosChecksumKey &&
      sanitized.PayosChecksumKey !== initial.PayosChecksumKey
    ) {
      updates.push({
        key: 'PayosChecksumKey',
        value: sanitized.PayosChecksumKey,
      })
    }

    if (sanitized.PayosReturnURL !== initial.PayosReturnURL) {
      updates.push({ key: 'PayosReturnURL', value: sanitized.PayosReturnURL })
    }

    if (sanitized.PayosCancelURL !== initial.PayosCancelURL) {
      updates.push({ key: 'PayosCancelURL', value: sanitized.PayosCancelURL })
    }

    if (sanitized.PayosUnitPrice !== initial.PayosUnitPrice) {
      updates.push({ key: 'PayosUnitPrice', value: sanitized.PayosUnitPrice })
    }

    if (sanitized.PayosMinTopUp !== initial.PayosMinTopUp) {
      updates.push({ key: 'PayosMinTopUp', value: sanitized.PayosMinTopUp })
    }

    if (sanitized.NowpaymentsApiUrl !== initial.NowpaymentsApiUrl) {
      updates.push({
        key: 'NowpaymentsApiUrl',
        value: sanitized.NowpaymentsApiUrl,
      })
    }
      ...
      <Form {...form}>
        ...
        >
          ...
          <Tabs defaultValue='general' className='min-w-0'>
            ...
            <TabsContent value='payos' className={paymentTabContentClassName}>
              <div className='space-y-4'>
                <div>
                  <h3 className='text-lg font-medium'>{t('PayOS Gateway')}</h3>
                  <p className='text-muted-foreground text-sm'>
                    {t('Configuration for PayOS payment integration (VN bank QR)')}
                  </p>
                </div>
                ...
                <div className='rounded-md bg-blue-50 p-4 text-sm text-blue-900 dark:bg-blue-950 dark:text-blue-100'>
                  ...
                  <ul className='list-inside list-disc space-y-1'>
                    <li>
                      {t('Webhook URL:')}{' '}
                      <code className='rounded bg-blue-100 px-1 py-0.5 text-xs dark:bg-blue-900'>
                        {'<ServerAddress>/api/payos/webhook'}
                      </code>
                    </li>
                    <li>{t('Configure in your PayOS dashboard')}</li>
                  </ul>
                </div>
                ...
                <div className='grid gap-6 md:grid-cols-2'>
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder={t('Enter PayOS client ID')}
                            {...field}
                            onChange={(event) =>
                              field.onChange(event.target.value)
                            }
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            type='password'
                            placeholder={t('Enter PayOS API key')}
                            autoComplete='new-password'
                            {...field}
                            onChange={(event) =>
                              field.onChange(event.target.value)
                        ...
                        <FormDescription>
                          {t('PayOS API key (leave blank unless updating)')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('Checksum Key')}</FormLabel>
                        <FormControl>
                          <Input
                            type='password'
                            placeholder={t('Enter PayOS checksum key')}
                            autoComplete='new-password'
                            {...field}
                            onChange={(event) =>
                              field.onChange(event.target.value)
                        ...
                        <FormDescription>
                          {t(
                            'PayOS checksum key (leave blank unless updating)'
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t(
                            'VND charged per 1 USD of top-up amount (e.g. 26000)'
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t('Leave blank to redirect back to the wallet page')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('Cancel URL')}</FormLabel>
                        <FormControl>
                          <Input
                            placeholder={t(
                              'Optional redirect after cancelled payment'
                            )}
                            {...field}
                            onChange={(event) =>
                              field.onChange(event.target.value)
            ...
            >
              <div className='space-y-4'>
                <div>
                  ...
                  <p className='text-muted-foreground text-sm'>
                    {t(
                      'Configuration for NOWPayments crypto payment integration (BTC/ETH/USDT...)'
                    )}
                  </p>
                </div>
                ...
                <div className='rounded-md bg-blue-50 p-4 text-sm text-blue-900 dark:bg-blue-950 dark:text-blue-100'>
                  ...
                  <ul className='list-inside list-disc space-y-1'>
                    ...
                    <li>
                      {t(
                        'The IPN callback URL is sent automatically with each invoice'
                      )}
                    </li>
                    <li>{t('Generate an IPN secret key in your NOWPayments dashboard')}</li>
                  </ul>
                </div>
                ...
                <div className='grid gap-6 md:grid-cols-2'>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('IPN Secret Key')}</FormLabel>
                        <FormControl>
                          <Input
                            type='password'
                            placeholder={t('Enter IPN secret key')}
                            autoComplete='new-password'
                            {...field}
                            onChange={(event) =>
                              field.onChange(event.target.value)
                        ...
                        <FormDescription>
                          {t('IPN secret key (leave blank unless updating)')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder={t(
                              'e.g. btc, eth, usdt (leave blank to let users choose)'
                            )}
                            {...field}
                            onChange={(event) =>
                              field.onChange(event.target.value)
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t('USD charged per 1 USD of top-up amount (1 = no markup)')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
