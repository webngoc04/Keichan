...
export function buildRateLimits(model: PricingModel): RateLimit[] {
  ...
  let baseRpm = 500
  if (isHeavy) baseRpm = 60
  else if (isLight) baseRpm = 5_000
  let baseTpm = 200_000
  if (isHeavy) baseTpm = 0
  else if (isLight) baseTpm = 1_000_000
  let baseRpd = 10_000
  if (isHeavy) baseRpd = 1_000
  else if (isLight) baseRpd = 100_000
