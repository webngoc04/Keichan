 ...
 * - Near end: [1, '...', 9, 10]
 */
export type PageNumberItem = { key: string; value: number | string }
...
export function getPageNumbers(
  ...
  if (totalPages <= maxVisiblePages) {
    for (let i = 1; i <= totalPages; i++) {
      rangeWithDots.push({ key: `page-${i}`, value: i })
    }
  } else {
    rangeWithDots.push({ key: 'page-1', value: 1 })

    if (currentPage <= 2) {
      rangeWithDots.push({ key: 'page-2', value: 2 })
      rangeWithDots.push({ key: 'ellipsis-start', value: '...' })
      rangeWithDots.push({ key: `page-${totalPages}`, value: totalPages })
    } else if (currentPage >= totalPages - 1) {
      rangeWithDots.push({ key: 'ellipsis-start', value: '...' })
      rangeWithDots.push({ key: `page-${totalPages - 1}`, value: totalPages - 1 })
      rangeWithDots.push({ key: `page-${totalPages}`, value: totalPages })
    } else {
      rangeWithDots.push({ key: 'ellipsis-start', value: '...' })
      rangeWithDots.push({ key: `page-${currentPage}`, value: currentPage })
      rangeWithDots.push({ key: 'ellipsis-end', value: '...' })
      rangeWithDots.push({ key: `page-${totalPages}`, value: totalPages })
    }
  }
