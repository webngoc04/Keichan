    ...
    return requiredTextParts.reduce<{
      ...
      (acc, part) => {
        if (part.type !== 'input') {
          return {
            ...acc,
            parts: [...acc.parts, { ...part, key: `part-${acc.partIndex}` }],
            partIndex: acc.partIndex + 1,
          }
        }
        return {
          parts: [
            ...acc.parts,
            { ...part, inputIndex: acc.inputIndex, key: `part-${acc.partIndex}` },
          ],
          inputIndex: acc.inputIndex + 1,
          partIndex: acc.partIndex + 1,
        }
      },
      { parts: [], inputIndex: 0, partIndex: 0 }
    ).parts
  }, [requiredTextParts])
