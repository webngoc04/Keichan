...
func RechargePayos(tradeNo string, callerIp string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if quotaToAdd > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("使用PayOS充值成功，充值额度: %v，支付金额: %.2f", logger.FormatQuota(quotaToAdd), topUp.Money), callerIp, topUp.PaymentMethod, PaymentProviderPayos)
	}

	return nil
}

func RechargeNowpayments(tradeNo string, actualAmount float64, callerIp string) (err error) {
	if tradeNo == "" {
		return errors.New("未提供支付单号")
	}
	...
	if quotaToAdd > 0 {
		RecordTopupLog(topUp.UserId, fmt.Sprintf("使用NOWPayments充值成功，充值额度: %v，实付加密货币等值: %.6f", logger.FormatQuota(quotaToAdd), actualAmount), callerIp, topUp.PaymentMethod, PaymentProviderNowpayments)
	}
